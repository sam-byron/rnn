...
class RNN(nn.Module):
    ...
    def forward(self, src_lang, target_lang, src_lengths):
        ...
        queries = self.query(src_embedding)
        keys = self.key(src_embedding)
        values = self.value(src_embedding)
        scores = torch.bmm(queries, keys.transpose(1, 2)) / (self.input_dim ** 0.5)
        attention = self.softmax(scores)
        weighted_input = torch.bmm(attention, values)
        out = nn.utils.rnn.pack_padded_sequence(weighted_input, src_lengths, enforce_sorted=False, batch_first=True)
