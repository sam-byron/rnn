...
class RNN(nn.Module):
    def __init__(self, vocab_size, target_vocab_size, embed_dim, target_embed_dim, rnn_hidden_size, batch_size, device):
        ...
        self.query = nn.Linear(embed_dim, 2*rnn_hidden_size)
        self.key = nn.Linear(2*rnn_hidden_size, 2*rnn_hidden_size)
        self.value = nn.Linear(2*rnn_hidden_size, 2*rnn_hidden_size)
        self.softmax = nn.Softmax(dim=2)
        self.fc = nn.Linear(2*2*rnn_hidden_size, target_vocab_size)
    ...
    def forward(self, src_lang, target_lang, src_lengths):
        ...
        queries = self.query(target_embedding)
        keys = self.key(out)
        values = self.value(out)
        scores = torch.bmm(queries, keys.transpose(1, 2)) / (self.input_dim ** 0.5)
        attention = self.softmax(scores)
        contextual_hidden_input = torch.bmm(attention, values)
        ...
        # out, _ = nn.utils.rnn.pad_packed_sequence(out, batch_first=True)
        hidden_and_context = torch.cat((out, contextual_hidden_input), dim = 2)
        hidden_and_context = nn.Dropout(p=0.5)(hidden_and_context)
        out = self.fc(hidden_and_context)
