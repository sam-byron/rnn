...
def encode_transform_batch(source_vocab, target_vocab, device):
    ...
    def collate_fn(batch):
        ...
        return padded_src_list.to(device), padded_target_list.to(device)
    
    return collate_fn

batch_size = 64
collate_fn = encode_transform_batch(en_vocab, spa_vocab, device)
...
class RNN(nn.Module):
    ...
    def forward(self, src_lang, target_lang, hidden, cell):
        src_embedding = self.embedding(src_lang)
        out, (hidden, cell) = self.encoder_rnn(src_embedding)
        ...
        out, (hidden, cell) = self.decoder_rnn(target_embedding, (hidden, cell))
        out = self.fc(out)
...
src_batch_summary = src_batch.to(device)
target_batch_summary = target_batch.to(device)
...
def train_procedure(dataloader, model, optimizer):
    ...
    for src_batch, target_batch in dataloader:
        ...
        optimizer.zero_grad()
        hidden, cell = model.init_hidden(batch_size)
        pred, hidden, cell = model(src_batch, target_batch, hidden, cell)
...
if TRAIN:
    ...
    for epoch in range(num_epochs):
        loss = train_procedure(train_dl, model, optimizer)
        val_loss = train_procedure(valid_dl, model, optimizer)
...
def sample(model, en_sentence, scale_factor=1.0):
    ...
    dummy_target = torch.tensor(target_text_transform(dummy_target))

    seq_length = 10
    next_word = dummy_target
    spa_sentence = ""
    hidden, cell = model.init_hidden(batch_size)
    for i in range(10):
        translation_logits, hidden, cell = model(en_sentence, next_word, hidden, cell)
